 ...
 */
bool Lz4Compression::performTests(float& cspeed, float& dspeed)
{
	try
	{
        std::string cstring;
		cspeed = performCompressionTest(cstring);
		dspeed = performDecompressionTest(cstring);
	}
 ...
 * \return  - speed MB/s
 */
//float Lz4Compression::performCompressionTest()
//{
//	//meassure();
//	auto size = 0;
...
float Lz4Compression::performCompressionTest(std::string &cstring)
{
    ...
    cstring = encode(buffer);//, csize);
    auto timeend = getTimeMillis();
    auto timeDif = timeend - timestart;
    printf("Compressed length = %.3f MB ", static_cast<float>(cstring.length()) / static_cast<float>(MB_BYTES));
    printf("Total compression time = %.3f ", timeDif / static_cast<float>(SEC_MILI));
    auto res = static_cast<float>(SEC_MILI) * static_cast<float>(MB_SIZE) / timeDif;
    //printf("%f MB/s ", res);
    return res;
}
 ...
 * \return - speed MB/s
 */
float Lz4Compression::performDecompressionTest(std::string &cstring)
{
    const int MB_BYTES = 1000000; //1 MB = 1000000 Bytes
    const int SEC_MILI = 1000; //1 second = 1000 milis
    
    auto timestart = getTimeMillis();
    auto dstring = decode(cstring);
    ...
    auto res = static_cast<float>(SEC_MILI) * (static_cast<float>(dstring.length()) / static_cast<float>(MB_BYTES)) / timeDif;
    return res;
}
...
//		printf("File = %s, loop = %d, %zu, %zu, %.f, %.f", filename.c_str(), iLoop, totalFileSize, totalCompressSize
...
//
//	printf("TOTAL %zu, %zu, %.f, %.f", totalFileSize, totalCompressSize
//	       , totalCompressTime, totalDecompressTime);
